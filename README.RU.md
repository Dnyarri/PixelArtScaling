...
> [!CAUTION]
> *Вынимание:* batch-программа перезаписывает исходные PNG, PPM и PGM, если хотите сделать копию - делайте это заранее! Это сделано нарочно, на случай, если вы решите улучшить разрешение печатного проекта с прилинкованными PNG - ScaleNxGUI при сохранении PNG умножает исходное разрешение на коэффициент масштабирования, так что итоговый печатный размер сохраняется в точности.
>
> Batch-программы в этой версии используют асинхронный мультипроцессинг, который резко уменьшает время обработки ценой загрузки всех процессоров на 100% и зажирания памяти на несколько файлов одновременно. Всё это делает интерфейс несколько неотзывчивым. Если вам кажется, что программа зависла - не пугайтесь, просто подождите, пока она пропихнёт все потоки со всеми вашими файлами через все процессоры разом.  

[**Scaler_Vi.py**](https://github.com/Dnyarri/PixelArtScaling/blob/main/Scaler_Vi.py) - недавно добавленная программа для масштабирования одной картинки с превью. Позволяет легко сравнивать разные алгоритмы, а после сохранения результата масштабировать его ещё дальше. Однако следует учитывать, что при подобном повторном масштабировании размер картинки растёт в геометрической прогрессии с каждым новым шагом, и, как сказал бы наш ротный, процесс быстро становится медленным.
